...
class UI:
    ...
    def end_Directory(self, content="videos", sortMethod=["none"], updateListing=False):
        if content:
    ...
    def showCategories(self):
        ...
        def add_category_item(title_id, blurb_id, mode, args={}):
            ...
            li = self.create_ListFldr(
                self.localized_string(title_id),
...
class PlayVideoAction(Action):
    def __init__(self, ui, *args, **kwargs):
        super(PlayVideoAction, self).__init__("play_video", ["url"], *args, **kwargs)
        self.ui = ui
...
class TalkPageAction(Action):
    def __init__(self, ui, *args, **kwargs):
        super(TalkPageAction, self).__init__("talkpage", ["url"], *args, **kwargs)
        self.ui = ui
...
class PlaylistAction(Action):
    def __init__(self, ui, *args, **kwargs):
        super(PlaylistAction, self).__init__("playlist", ["url"], *args, **kwargs)
        self.ui = ui
...
class SearchAction(Action):
    ...
    def create_search_action_listitem(self, page, cat):
        def S(id):
            return self.ui.localized_string(id)
    ...
    def run_internal(self, args):
        ...
        if page < 0:
            q = xbmcgui.Dialog().yesnocustom(
                ...
                self.ui.localized_string(30005),
            )  # search (playlist)
            if q < 0 or q > 1:
        ...
        if not search_string:
            self.ui.add_DirectoryItem(*self.create_search_action_listitem(0, cat))
            if bool(settings.get_current_search()):
                self.ui.add_DirectoryItem(*self.create_search_action_listitem(1, cat))
        ...
        for title, url, thumb, plot in generator:
            ...
            li = self.ui.create_ListFldr(
                title, thumb, {"title": title, "plot": plot, "mediatype": "video"}
            )
...
class RssFeedsAction(Action):
    ...
    def talkpage_menu_item(self, url):
        return (
            self.ui.localized_string(30091),
    ...
    def run_internal(self, args):
        ...
        for talk in feeds.get_new_talks(url if url != "latest" else None):
            ...
            self.ui.add_DirectoryItem(url, li, False)
        self.ui.end_Directory("videos", ["dateadded", "date", "title"])
